...
interface TShapeTemplateProps {
  isSelected: boolean
  onSelect: (shape: LayoutShape) => void
}

export function TShapeTemplate({ isSelected, onSelect }: TShapeTemplateProps) {
  return (
    <motion.div
      whileHover={{ scale: 1.02, y: -2 }}
      whileTap={{ scale: 0.98 }}
      onClick={() => onSelect('t-shape')}
      className={`
    ...
    >
      ...
      <span className="font-bold uppercase text-sm">T-SHAPE</span>
    </motion.div>
  )
}