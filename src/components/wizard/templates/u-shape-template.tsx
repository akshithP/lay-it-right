...
interface UShapeTemplateProps {
  isSelected: boolean
  onSelect: (shape: LayoutShape) => void
}

export function UShapeTemplate({ isSelected, onSelect }: UShapeTemplateProps) {
  return (
    <motion.div
      whileHover={{ scale: 1.02, y: -2 }}
      whileTap={{ scale: 0.98 }}
      onClick={() => onSelect('u-shape')}
      className={`
    ...
    >
      ...
      <span className="font-bold uppercase text-sm">U-SHAPE</span>
    </motion.div>
  )
}